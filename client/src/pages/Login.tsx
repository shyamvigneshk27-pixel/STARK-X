...
const fieldVariants = {
  ...
  animate: (i: number) => ({ opacity: 1, x: 0, transition: { delay: i * 0.1, duration: 0.4 } }),
};
      ...
      <div className="hidden lg:flex flex-col flex-1 items-center justify-center relative overflow-hidden p-8">
        ...
        <div className="relative z-10 w-full max-w-md">
          ...
          <div className="text-center mt-6">
            <h1 className="text-4xl font-black gradient-text leading-tight">
              Your next adventure
            </h1>
            <p className="text-white/60 mt-2 text-lg">starts here.</p>

            {/* Floating trip cards */}
            {['Japan Cherry Blossom 🌸', 'Bali Paradise 🌴', 'Paris Romance 🗼'].map((label, i) => (
              <motion.div
                key={label}
                animate={{ y: [0, -8, 0] }}
                transition={{ duration: 3 + i, repeat: Infinity, delay: i * 1.2 }}
                className="inline-block glass rounded-xl px-4 py-2 text-sm text-white/80 mx-1 mt-4"
              >
                {label}
              </motion.div>
            ))}
          ...
          <div className="glass rounded-3xl p-8 shadow-glow">
            ...
            <div className="mt-6 text-center space-y-3">
              ...
              <div className="text-xs text-white/20">
                Demo: demo@traveloop.com / Demo@1234
              </div>
            </div>
          </div>
        </div>
